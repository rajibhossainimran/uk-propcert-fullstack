  ...
  return (
    <div className={`fixed md:relative md:block w-64 bg-white h-screen p-4 shadow-lg transform transition-transform duration-300 ${
      ...
      <nav className="space-y-2">
        <Link to="/mydashboard/dashboard" className={`w-full flex items-center space-x-2 p-2 rounded-lg ${activeTab === 'dashboard' ? 'bg-lime-100 text-lime-700' : 'text-gray-600 hover:bg-gray-100'}`} onClick={() => setActiveTab('dashboard')}>
          <FiHome className="w-5 h-5" />
          <span>Dashboard</span>
        </Link>
        <button className={`w-full flex items-center space-x-2 p-2 rounded-lg ${activeTab === 'documents' ? 'bg-lime-100 text-lime-700' : 'text-gray-600 hover:bg-gray-100'}`} onClick={() => setActiveTab('documents')}>
